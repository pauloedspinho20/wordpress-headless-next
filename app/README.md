...
[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https://github.com/vercel/next.js/tree/canary/examples/cms-wordpress&project-name=cms-wordpress&repository-name=cms-wordpress&env=NEXT_PUBLIC_WORDPRESS_GRAPHQL_URL&envDescription=Required%20to%20connect%20the%20app%20with%20WordPress&envLink=https://vercel.link/cms-wordpress-env)
...
```bash
npx create-next-app --example cms-wordpress app
```

```bash
yarn create next-app --example cms-wordpress app
```

```bash
pnpm create next-app --example cms-wordpress app
```
...
Then open `.env.local` and set `NEXT_PUBLIC_WORDPRESS_GRAPHQL_URL` to be the URL to your GraphQL endpoint in WordPress. For example: `https://myapp.wpengine.com/graphql`.
...
- Uncomment `NEXT_PUBLIC_WORDPRESS_AUTH_REFRESH_TOKEN` and set it to be the `refreshToken` you just received.
- Uncomment `NEXT_PUBLIC_WORDPRESS_PREVIEW_SECRET` and set it to be any random string (ideally URL friendly).
...
- `<secret>` should be the string you entered for `NEXT_PUBLIC_WORDPRESS_PREVIEW_SECRET`.
